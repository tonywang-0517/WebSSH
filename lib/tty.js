...
//////////
  ...
  this.use(term.middleware());
    this.use(session({secret:'asdf',resave:'true',saveUninitialized:'true'}));
  if (conf.static) {
...
Server.prototype.initRoutes = function() {
  ...
  this.get('/options.js', function(req, res, next) {
    ...
    this.post('/terminal', function(req, res, next) {
    });
    this.get('/terminal', function(req, res, next) {
        if(req.session.ip&&req.session.user&&req.session.password)
        {res.render("../static/textbox.ejs",{ip:req.session.ip,user:req.session.user,password:req.session.password});}
        else
        {res.redirect("/"); }
...
Server.prototype.handleConnection = function(socket) {
  ...
  socket.on('create', function(cols, rows, func) {
    ...
    /////////////////////////////////////////
    //增加登陆重连功能
    socket.on('reconnect', function(cols, rows, func) {
        return session.handleCreate(cols, rows, func);
    });
...
Session.prototype.handleCreate = function(cols, rows, func) {
  ...
  term.on('data', function(data) {    //一会记得实现用bool控制判断输出
      console.log(data);
...
Session.prototype.handleData = function(id, data) {
  ...
  terms[id].write(data); //term.js向ssh写命令
};
